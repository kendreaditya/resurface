...
import { renderBlockContent } from "./mdParser";
...
function sectionHeader(count: number): string {
  ...
  return `
    <div class="content">
      <div class="flex-1 flex-row foldable-title cursor">
...
function renderCard(item: Resurfaced): string {
  ...
  const bodyClass = collapsed ? "initial hidden" : "initial";
  ...
  const blockTree = renderRoot(tree, block.uuid);
...
export function renderResurfaced(items: Resurfaced[]): { html: string; count: number } {
  if (items.length === 0) return { html: "", count: 0 };

  const bodyClass = state.sectionCollapsed ? "initial hidden" : "initial";
...
function renderRoot(block: BlockEntity, fallbackUuid: string): string {
  const rootContent = block.content ?? block.title ?? "";
  const rootRendered = renderBlockContent(rootContent);
  const rootHasContent = !!(rootRendered.titleHtml || rootRendered.bodyHtml);
  const children = getChildren(block);
  if (!rootHasContent && children.length > 0) {
    return children
      .map((child) => renderBlockNode(child, 1, child.uuid))
      .join("");
  }
  return renderBlockNode(block, 1, fallbackUuid);
}

function renderBlockNode(block: BlockEntity | null, level: number, fallbackUuid: string): string {
  const uuid = block?.uuid ?? fallbackUuid;
  const content = block?.content ?? block?.title ?? "";
  const children = getChildren(block);
  const hasChildren = children.length > 0;
  const { titleHtml, bodyHtml, hasHeading } = renderBlockContent(content);
        ...
        <div class="flex flex-col block-content-wrapper">
          <div class="flex flex-row">
            <div class="flex-1 w-full" style="display: flex;">
              <div id="${contentId}" blockid="${uuidEsc}" data-type="default" class="block-content inline" style="width: 100%;">
                <div class="flex flex-row justify-between block-content-inner">
                  <div class="flex-1 w-full">${titleHtml}</div>
                </div>
                ${bodyHtml ? `<div class="block-body">${bodyHtml}</div>` : ""}
              </div>
            </div>
      ...
      ${hasChildren ? `
        <div class="block-children-container flex">
          ...
          <div class="block-children w-full">
            ${children.map((child) => renderBlockNode(child, level + 1, child.uuid)).join("")}
          </div>
        </div>
      ` : ""}
    </div>
